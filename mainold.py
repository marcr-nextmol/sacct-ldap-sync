...
def add_account_and_assoc(group: str, cluster: str, org: str) -> V0041OpenapiResp:
    response=None
    try:
        print(f"Create account and association for {group} on cluster {cluster}")
        V0041_openapi_associations = {
            "associations": [
                {
                    "account": group,
                    "cluster": cluster,
        ...
        response = SlurmdbApi.slurmdb_V0041_post_accounts_and_associations(
            V0041_openapi_associations_resp=V0041_openapi_associations
        )
        return response
    
    except Exception as e:
        print(f"Error creating account/association: {e}")
    finally: 
        return response
...
if __name__ == "__main__":
    ...
    for group, users_list in dict_groups.items():      
        ...
        for user in users_list:
            response_account = add_account_and_assoc(group,CLUSTER, ORG)
